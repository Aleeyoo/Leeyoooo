...
DEEPSEEK_ENV="ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic ANTHROPIC_AUTH_TOKEN=sk-ca71f60269524bf08fba78910b82c4a5 ANTHROPIC_MODEL=deepseek-v4-pro CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1"
...
run_output_action() {
  ...
  (cd "$VAULT_ROOT" && env $DEEPSEEK_ENV "$CLAUDE_BIN" -p "$prompt") > "$out"
  echo "OUTPUT:${out#$VAULT_ROOT/}"
}
...
EOF
  ...
  (cd "$VAULT_ROOT" && env $DEEPSEEK_ENV "$CLAUDE_BIN" -p "$prompt")
  echo "OUTPUT:$NOTE_REL"
}
