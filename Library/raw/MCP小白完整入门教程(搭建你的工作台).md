...
![](https://pbs.twimg.com/media/HJOwGRrbQAAApuz.jpg)
...
[https://abs.twimg.com/emoji/v2/svg/1f3c6.svg](https://abs.twimg.com/emoji/v2/svg/1f3c6.svg)
...
要 API Key，去 firecrawl.dev 注册，免费额度够个人玩。

把 fc-YOUR_API_KEY 换成你自己的。
...
它要一个 GitHub Personal Access Token，去 github.com/settings/tokens 申请。
...
- @mcp.tool() 装饰器把一个普通函数注册成 MCP 的 Tool。函数的参数类型注解（a: float, b: float）和 docstring 会自动转换成 MCP 协议里的工具 schema 和描述——AI 就是靠这些来判断"这个工具是干什么的、要传什么参数"。
- mcp.run() 用默认的 stdio 传输启动 Server。Claude Code 会通过标准输入输出和它通信。
...
> 注意：Registry 当前是 **preview 阶段**，规范和命令都可能变。下面流程基于 2025-12-11 版的 server.json schema，跟官方 modelcontextprotocol.io/registry/quickstart 走最稳。
这一章你会做的 5 件事
...
- **packages[].registryType**：底层包发在哪。npm / pypi / oci（Docker）三选一。
...
3. 点 Authorize
回到终端看到 Successfully authenticated! 就成了。
...
看到 ✓ Successfully published 就完事了。
...
坑一：Registry validation failed for package
...
坑二：Invalid or expired Registry JWT token

- **症状**：上架时报 JWT 过期
- **原因**：登录有时效，过期了
- **解法**：跑一遍 mcp-publisher login github 重新登
坑三：You do not have permission to publish this server
...
官方提供了 GitHub Actions 模板，每次 git tag 自动上架。文档在 modelcontextprotocol.io/registry/github-actions。
...
- docs/develop/build-server — 写 Server 的官方教程
- docs/tools/inspector — Inspector 调试工具用法
微软 mcp-for-beginners 课程

仓库地址： github.com/microsoft/mcp-for-beginners。
...
**啥时候需要**：你不想用 io.github.xxx/，想用 com.你公司域名/ 上架，看起来更专业、更品牌化。

需要走 DNS 认证流程：在你域名的 DNS 里加一条 TXT 记录证明所有权。

文档在 registry/authentication。
 ...
 **黄小木｜T11级架构师｜内容为ai辅助创作，若有侵权请指出｜持续分享 AI 信息、副业赚钱、程序员转型OPC心得｜X：**@ai_xiaomu

https://x.com/ai_xiaomu/article/2059202018682831129

— [黄小木 (@ai_xiaomu)](https://x.com/ai_xiaomu/status/2059202018682831129) · 2026-05-26 17:16
