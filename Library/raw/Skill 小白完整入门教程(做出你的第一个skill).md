---
status: processed
wiki: "[[WIKI/Skill小白入门教程]]"
---

# X 上的 黄小木：“Skill 小白完整入门教程(做出你的第一个skill)” / X

![图像](https://pbs.twimg.com/media/HJAMusYaAAEB4RX?format=jpg&name=large)
https://x.com/ai_xiaomu/status/2058360688293331143?s=20
...
很多人听到 Claude Skills 就以为这是 Claude 专属功能,其实**完全不是**。
...
文章下面会以 Claude Code 为主举例(因为它是标准制定者,生态最全),但**所有原理、写法、避坑经验,对所有支持 Agent Skills 的 AI 工具都通用**。
...
![图像](https://pbs.twimg.com/media/HI_fFr3agAAuMJV?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HI_fGiOaMAAWp0p?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HI_fHZmbIAABDNC?format=jpg&name=large)
...
2. 根据 description 做**语义判断**,不是关键字匹配
...
![图像](https://pbs.twimg.com/media/HI_fItdbUAAldBf?format=jpg&name=large)

**自由度怎么把握**:

![图像](https://pbs.twimg.com/media/HI_fJzXagAAwhow?format=jpg&name=large)

五种设计模式

Anthropic 从早期使用者归纳出五种 Skill 设计模式:

![图像](https://pbs.twimg.com/media/HI_fLNDbkAAjxTd?format=jpg&name=large)
...
唯一的例外是**输出格式：**"输出一定要用这个模板"这种机械要求,没有"为什么"可解释,直接写死。
...
别加 README.md、INSTALLATION\_GUIDE.md、QUICK\_REFERENCE.md、CHANGELOG.md 这些人类文档,它们只会浪费 context。
...
![图像](https://pbs.twimg.com/media/HI_fL-7bAAA3BXy?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HI_fMr5bYAAWQ8Q?format=jpg&name=large)

**经验**:除非项目特定,都放个人级 ~/.claude/skills/ 统一管理,避免忘记自己有哪些 Skill。

三种安装方式：

![图像](https://pbs.twimg.com/media/HI_fNmBa8AAyTTl?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HI_fOZQbwAAvnh8?format=jpg&name=large)
...
- **CC Switch**: 开源工具管理多 API 配置一键切换 [github.com/farion1231/cc-switch](https://github.com/farion1231/cc-switch)
- **原生安装比 npm 稳**: curl -fsSL [https://claude.ai/install.sh](https://claude.ai/install.sh) | bash
...
**协作模式**:主 Skill 在 ## 步骤 里**显式调用**其他 Skill:
...
![图像](https://pbs.twimg.com/media/HI_fPbkbAAAv8NJ?format=jpg&name=large)

五条工程经验

1. **粒度要细**:每个 Skill 一个明确任务
2. **协作显式化**:主 Skill 用 ## 步骤 显式调用其他 Skill
3. **脚本承担计算**:SEO 字符数、链接统计这类需要**客观数据**的部分一定用脚本,不要让模型估算
...
2. 同时跑 with\_skill 和 baseline(无 skill),双盲对比
3. 通过 agents/grader.md 给每个断言打分
4. 用 aggregate\_benchmark 出 pass\_rate / time / tokens 报告
...
4. **看 transcript 找重复工**:如果每个 case 子代理都独立写了 create\_docx.py,说明应该 bundle 进 scripts/
...
![图像](https://pbs.twimg.com/media/HI_fQgdaIAEtnqd?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HI_fXqXbEAAk5ZL?format=jpg&name=large)

## 十、生态与必装清单

Skill 资源地图

![图像](https://pbs.twimg.com/media/HI_fYbKa0AA3aOc?format=jpg&name=large)

必装 Skill 清单

![图像](https://pbs.twimg.com/media/HI_fW9jbcAArmEP?format=jpg&name=large)
