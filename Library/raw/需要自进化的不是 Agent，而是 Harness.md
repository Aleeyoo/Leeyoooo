---
status: processed
wiki: "[[WIKI/Harness自进化]]"
---
# 需要自进化的不是 Agent，而是 Harness

![](https://pbs.twimg.com/media/HJTA64abwAAVoxp.jpg)
...
这是 Phil Schmid （我所知道的，最早提出 [Harness 概念](https://www.philschmid.de/agent-harness-2026) 的人）所说的「持久性（Durability）」 的体现 —— 模型在执行数百次工具调用后，还能多好地遵循指令？一个模型在 benchmark 上领先 1%，并不能说明它在第 50 步不会误删文件、忘记约束，或者在工具失败后开始编造结果。

但这里有一个悖论：模型越来越强，构建在其上的产品体验却未必同步提升。为什么？

因为**缺失了一环**：产品本身能否随模型迭代、用户使用而自动进化。

## 什么是 Harness？为什么它必须是活的？

在进入正题之前，我们需要先理解一个概念：**Agent Harness**。

![](https://pbs.twimg.com/media/HJTBCpIakAAL_Vr.jpg)
...
![](https://pbs.twimg.com/media/HJTBJU2aEAAKmwn.jpg)
...
![](https://pbs.twimg.com/media/HJTCMshbEAAt4Om.jpg)
...
![](https://pbs.twimg.com/media/HJTCerqa0AA6wNB.jpg)

![](https://pbs.twimg.com/media/HJTChTDaAAAj4zB.jpg)
...
![](https://pbs.twimg.com/media/HJTCp2-aUAA_Hd2.jpg)
...
![](https://pbs.twimg.com/media/HJTCudHbQAArPzh.jpg)

## Harness 自进化的四个层次

![](https://pbs.twimg.com/media/HJTCxcabcAAwDA6.jpg)
...
而下一步 **L4 的预期形态**是：Agent 还能主动优化 Context Engine 的策略、调整 Tool schema 的兼容层、预测和预防错误——**Harness 自己进化自己，人只需要设定一个目标**，而我们正在实现的路上。

## 为什么 Consumer-Aimed Product 本质上是 Self-Evolving 的 Harness？

这个实践让我们看到了一个更大的图景。

**传统 SaaS 产品**的逻辑是：产品 = 功能代码 + 用户数据。功能是固定的，用户数据是副产品。产品变好靠的是版本更新——人写代码、人测试、人发布。

**AI-Native 产品**的逻辑则完全不同：产品 = Harness（运行时）+ 用户交互数据 + 进化能力。每一次用户交互都在为 Harness 提供进化信号——不是训练模型，而是优化运行时。

这意味着：

1. **真正的护城河不是模型，而是 Harness 中沉淀的上下文、pattern、trajectory**
2. 模型是通用的、外部的、可替换的，Harness 是专属的、内部的、持续积累的。当你的 Harness 积累了数百个 error pattern、上万条 trajectory 数据、针对你的用户场景优化的 context strategy —— 这是竞争对手无法复制的
3. **产品体验随使用自动变好，而非等待版本更新**
4. 传统产品：用户遇到 bug → 反馈 → 等排期 → 等发布 → 更新 Self-Evolving 产品：Harness 自动发现 → Agent 自动分析 → Agent 自动修复 → 用户无感升级
5. **用户在使用产品的过程中，也在“训练”产品**
6. 每一次模型报错，都在丰富 error pattern 的执行回路 每一次 Context 溢出，都在优化 context compaction 策略 每一次 Tool call 失败，都在完善 schema 兼容层
...
![](https://pbs.twimg.com/media/HJTC6S_bYAAmWjj.jpg)
...
这就是 Self-Evolving Harness 的终极意义：它不是一个更聪明的静态系统，而是一个**能够随环境变化而自动适应的生命体**。
...
因为大多数产品仍然把 Harness 当作一个**静态的执行环境**——它不会学习，不会进化，不会自己变好。
...
（PS：我们的 Agent Tracing 已在 LobeHub 主仓库开源:[https://github.com/lobehub/lobehub/tree/canary/packages/agent-tracing](https://github.com/lobehub/lobehub/tree/canary/packages/agent-tracing) ，我们正在考虑是否将其作为独立项目发布，如果有任何想法或者反馈，欢迎找我们交流

https://x.com/arvin17x/article/2059489592097849698

— [空谷 Arvin Xu (@arvin17x)](https://x.com/arvin17x/status/2059489592097849698) · 2026-05-27 12:19
